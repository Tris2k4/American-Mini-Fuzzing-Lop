...
def parse_config(config_file, overwrite_output=True):
    with open(config_file) as f:
        ...
        if 'dictionary_file' in conf_dict:
            if not os.path.exists(conf_dict['dictionary_file']):
                print("Warning: Dictionary file does not exist")
                conf_dict['dictionary_file'] = None
        else:
            conf_dict['dictionary_file'] = None

        # Create crashes folder
        crashes_folder = os.path.join(conf_dict['output_folder'], 'crashes')
        if not os.path.exists(crashes_folder):
            os.makedirs(crashes_folder)
        conf_dict['crashes_folder'] = crashes_folder
